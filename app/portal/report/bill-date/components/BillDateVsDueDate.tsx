...
export default function BillDateVsDueDate({ station_type }: { station_type: string }) {
    ...
    const fetchDelayData = async () => {
        try {
            ...
            data.forEach((bill: any) => {
                ...
                const diffInDays = Math.floor((dueDate.getTime() - billDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const handleBarClick = (data: any) => {
        ...
        queryParams.append('bill_date_vs_due_date', data?.split(' ')[0]);
            ...
            <Card className="overflow-hidden border-2 bg-card transition-all duration-200 hover:border-primary/20">
                <CardHeader className="space-y-1 pb-4">
                    <div className="flex items-center justify-between">
                        <CardTitle className="text-2xl font-semibold">Bill Date vs Due Date</CardTitle>
                        {isLoading && (
                    ...
                    <p className="text-sm text-muted-foreground">
                        Distribution of bills based on the gap between bill date and due date
                    </p>
                </CardHeader>
                <CardContent>
                    {error ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-center">
                                <span className="text-sm text-muted-foreground">Loading analysis data...</span>
                            </div>
                        </div>
                    ) : delayData.length === 0 ? (
