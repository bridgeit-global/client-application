...
export default function AllOverBill({ station_type }: { station_type: string }) {
    ...
    const fetchBillData = async () => {
        try {
            ...
            const monthlyData = data.reduce((acc: {
                ...
                if (bill.connection_id) acc[monthYear].connectionIds.add(bill.connection_id);
                if (bill.connections && bill.connections.site_id) acc[monthYear].siteIds.add(bill.connections.site_id);
                return acc;
            }, {});
            ...
            const chartData = Object.entries(monthlyData)
                ...
                .map(([month, data]) => ({
                    ...
                    connectionCount: data.connectionIds.size,
                    siteCount: data.siteIds.size
                }));

            setBillData(chartData);
        } catch (err) {
            ...
            <Card className="overflow-hidden border-2 bg-card transition-all duration-200 hover:border-primary/20">
                <CardHeader className="space-y-1 pb-4">
                    <div className="flex items-center justify-between">
                        <CardTitle className="text-2xl font-semibold">Monthly Bill Trend</CardTitle>
                        {isLoading && (
                    ...
                    <p className="text-sm text-muted-foreground">
                        Visualizing Bill patterns across different time periods
                    </p>
                </CardHeader>
                <CardContent>
                    {error ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-center">
                                <span className="text-sm text-muted-foreground">Loading Bill data...</span>
                            </div>
                        </div>
                    ) : BillData.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-[400px] space-y-4 text-center">
                            ...
                            <div className="space-y-2">
                                <p className="text-xl font-medium">No Bill data available</p>
                                <p className="text-sm text-muted-foreground max-w-sm">
                                    Try adjusting your filters or selecting a different date range to view Bill patterns
                                </p>
                            </div>
                            <Button
                                variant="outline"
