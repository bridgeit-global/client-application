...
const mag4 = ['all', ['has', 'unit_cost'], ['>', ['get', 'unit_cost'], 10]];

export default function UnitCostMap({
  data,
  setActiveMap,
  activeMap
}: {
  ...
  useEffect(() => {
    const initializeMap = async () => {
      ...
      mapInstance.on('load', async () => {
        ...
        mapInstance.addSource('unit_cost_data', {
          type: 'geojson',
          data: data[mapKey],
          cluster: true,
          clusterRadius: 80,
        ...
        legend.innerHTML = `
          ...
          <div style="text-align: center; padding: 5px; background-color: ${colors[1]}">0 Units</div>
          <div style="text-align: center; padding: 5px; background-color: ${colors[2]}">0 - 5 Unit Cost</div>
          <div style="text-align: center; padding: 5px; background-color: ${colors[3]}">5 - 10 Unit Cost</div>
          <div style="text-align: center; padding: 5px; background-color: ${colors[4]}">10+ Unit Cost</div>
        `;
        ...
        mapInstance.on('render', () => {
          if (!mapInstance || !mapInstance.isSourceLoaded('unit_cost_data'))
            return;
          updateMarkers(
            mapInstance,
            markers.current,
            markersOnScreen.current,
            (props) => createDonutChart(props) as Node,
            'unit_cost_data'
          );
        });
        ...
        mapInstance.on('click', 'data-circle', (e) => {
          ...
          popup
            ...
            .setHTML(
              ...
              <p>Unit Cost: <span style="color: ${feature.properties?.unit_cost < 10 ? 'green' : 'red'
              };">${feature.properties?.unit_cost || 'No Data'}</span></p>
      ...
      <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:justify-between">
        ...
        {/* <div className="grid grid-cols-3 gap-2 sm:flex">
          <Button
            ...
            variant={mapKey === 'geojsonData' ? 'default' : 'outline'}
            onClick={() => setMapKey('geojsonData')}
          >
            Latest Month
          </Button>
          <Button
            size={'sm'}
            className="text-xs sm:text-sm"
            variant={mapKey === 'geojsonThreeMonthData' ? 'default' : 'outline'}
            onClick={() => setMapKey('geojsonThreeMonthData')}
          >
            Three Months
          </Button>
          <Button
            size={'sm'}
            className="text-xs sm:text-sm"
            variant={mapKey === 'geojsonSixMonthData' ? 'default' : 'outline'}
            onClick={() => setMapKey('geojsonSixMonthData')}
          >
            Six Months
          </Button>
        </div> */}
