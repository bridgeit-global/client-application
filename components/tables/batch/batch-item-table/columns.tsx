...
export const postpaidColumns: ColumnDef<AllBillTableProps>[] = [

  {
    id: 'batch_id',
    header: 'Batch ID',
    cell: ({ row }) => <ViewBatchButton link={`/portal/batch/${row.original.batches.batch_id}`} batchId={row.original.batches.batch_id} />
  },
  {
    id: 'id',
    header: () => useSiteName(),
...
export const prepaidColumns: ColumnDef<PrepaidRechargeTableProps>[] = [
  ...
  {
    header: 'Action',
    cell: ({ row }) => row.original.batches.batch_status === 'unpaid' ? <RemoveRechargeFromBatchButton row={row} /> : null
  }
];
