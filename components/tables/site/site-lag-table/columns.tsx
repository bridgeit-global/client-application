...
export const columns: ColumnDef<ConnectionTableProps>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (row.original.bills.length === 0) {
        return (
          <Badge className="w-32 truncate bg-gray-500 text-center text-white hover:bg-gray-600">
            No Bill
          </Badge>
        );
      }
  ...
  },
  {
    ...
    cell: ({ row }) => row.original.bills[0]?.bill_date && ddmmyy(row.original.bills[0].bill_date)
  },
  {
    header: 'Next Bill Date',
    cell: ({ row }) => row.original.next_bill_date && ddmmyy(row.original.next_bill_date)
  }
];
